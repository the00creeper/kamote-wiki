---
description: The fixed set of prices for rare commodities!
---

# 💲 Standard Retail Price

The **Standard Retail Price (SRP)** system establishes recommended prices for rare and valuable items in the server.&#x20;

Its purpose is to maintain a fair and balanced economy by preventing excessive overpricing, ensuring that essential rare items remain accessible to all players—especially newcomers who may not yet be familiar with market values.&#x20;

{% hint style="warning" %}
Selling items above the SRP is punishable under [_Kamnst. art. III, § 7_](../../the-justice-system/the-kamonstitution.md#article-iii-the-economy)_._
{% endhint %}

The table below lists the current SRP for selected items:

{% tabs %}
{% tab title="Sold Per Stack" %}
## Sold Per Stack

| Item                 | Price Per Stack |
| -------------------- | --------------- |
| Bottle of Enchanting | $15             |
| Breeze Rod           | $24             |
| Ender Pearl          | $5              |
| Gunpowder            | $4              |
| Paper                | $2              |
{% endtab %}

{% tab title="Sold Per Piece" %}
## Sold Per Piece

| Item                                | Price Per Piece |
| ----------------------------------- | --------------- |
| Beacon                              | $115            |
| Conduit                             | $50             |
| Elytra                              | $50             |
| Heavy Core                          | $100            |
| Netherite Ingot                     | $30             |
| Netherite Upgrade Smithing Template | $50             |
| Shulker Box                         | $15             |
| Silence Armor Trim                  | $150            |
| Swift Sneak                         | $200            |
| Totem of Undying                    | $2.5            |
| Trident                             | $50             |
| Windburst I                         | $30             |
{% endtab %}
{% endtabs %}
